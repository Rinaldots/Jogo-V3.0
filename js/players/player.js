...
class Player {
    ...
    lifeBar(){
        if(this.atualLife>=0){
            ...
            context.fillText(this.name, this.position.x+(this.size.width/2),this.position.y-enemyLifeBarY-20,2000)
        }
    ...
    AttackcheckHit(attacker,target){
        ...
        x1Right=attacker.attackBox.position.x+attacker.size.width+attacker.attackBox.size.width;
        x1Left=attacker.attackBox.position.x+attacker.size.width;
        }
    ...
    updateCamera(){
        //horizontal Move
        if(this.position.x <  this.camerabox.position.x+(this.camerabox.size.width/3))
        {
            this.camerabox.position.x -= this.camerabox.velocity
            camera.position.x += this.camerabox.velocity
        }

        if(this.position.x >  this.camerabox.position.x-this.size.width+(this.camerabox.size.width*2/3))
        {
            this.camerabox.position.x += this.camerabox.velocity
            camera.position.x -= this.camerabox.velocity
        }
        ...
        //vertical Move
        if(this.position.y > (this.camerabox.position.y+(this.camerabox.size.height)*5/8)){
            
            this.camerabox.position.y += maxSpeed+1
            camera.position.y -= maxSpeed+1
            
        }
        if(this.position.y < this.camerabox.position.y+(this.camerabox.size.height*4/8)){
